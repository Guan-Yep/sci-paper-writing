...
### Figure Embedding in Documents

**CRITICAL**: A figure is NOT complete until it is physically embedded in the output document. Textual references alone are insufficient.

**The three requirements for every figure**:

1. **Textual reference in body**: The paragraph must say "... as shown in Fig. N ..." or equivalent
2. **Physical embedding**: The image file must be inserted into the document object:
   - python-docx: `paragraph.add_run().add_picture(image_path, width=Inches(5.5))`
   - OpenXML SDK: `new Drawing(new InlinePicture(...))` appended to paragraph
   - LaTeX: `\includegraphics[width=\linewidth]{image_path}`
3. **Caption immediately below**: A centered bold caption paragraph follows the image

**Embedding checklist**:

- [ ] Image file exists at the specified path before embedding
- [ ] Image is embedded at the same location as the textual reference (not at end of document)
- [ ] Image width matches the column layout (see width table above)
- [ ] Caption is placed BELOW the image (not above)
- [ ] Caption is self-contained and understandable without reading the main text

**Common failure mode**: Writing "Fig. 2 shows the training curves" in the text but never calling `add_picture()`. The document will have a reference to a non-existent figure.

**Verification**: After generating the document, open it and scroll through. Every "Fig. N" in the text must have a corresponding image visible on the page. If not, the embedding step was skipped.
...
7. **Embed the image** in the document at the same location as the text reference
8. **Verify** that every "Fig. N" in the text has a corresponding embedded image
...
| Text reference without embedding | Always call `add_picture()` after "Fig. N" reference |
| Image at end of document         | Embed image immediately after text reference         |
